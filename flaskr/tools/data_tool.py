...
def loadData():
    ...
    return getBlogs(), getTopic(), getRates()
...
def getBlogs():
    rootPath = os.path.abspath(os.getcwd())
    path = f"{rootPath}/flaskr/static/blog_data/blog_info1.csv"
    df = pd.read_csv(path, delimiter="|")
    df.set_index('movieId')
    return df
...
def getTopic():
    rootPath = os.path.abspath(os.getcwd())
    path = f"{rootPath}/flaskr/static/blog_data/topic.csv"
    df = pd.read_csv(path, delimiter="|", names=["name", "id"])
    df.set_index('id')
    return df
...
def getRates():
    rootPath = os.path.abspath(os.getcwd())
    path = f"{rootPath}/flaskr/static/blog_data/Blog_Ratings.csv"
    df = pd.read_csv(path,delimiter=",", names=["movieId","userId","rating"])  # blog_id,userId,ratings
